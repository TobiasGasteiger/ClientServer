...
public class Client {
	public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            if(!answer.getStatus().equals("close"))
                writeFile(answer.getData(), readIn("Geben sie den Dateinamen an um die Datei auch lokal zu speichern", s, in, out) );
            else
                out.writeObject(new Acknowledgement(true));
    ...
    private static Answer handleConnection(Socket s, ObjectOutputStream out, ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        do{
            ...
            path = readIn("Pfad eingeben [kill server = Server schließen]", s, in, out);
    ...
    private static void writeFile(byte[] data, String path) {
        ...
        try {
            ...
            printError("Die Datei wurde nicht gefunden");
        } catch (IOException e) {
            printError("Fehler beim Schreiben der Datei");
        }
    }
